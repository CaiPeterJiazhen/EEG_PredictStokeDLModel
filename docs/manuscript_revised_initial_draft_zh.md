...
不确定性评估采用受试者层面 bootstrap 区间。配对比较采用受试者层面重采样。硬预测的配对比较采用 McNemar 检验。置换检验使用患者层面标签置换。校准性能通过 Brier 分数和校准曲线评估。最终锁定的患者层面预测用于生成混淆矩阵。FMA-UE 和 MBI 的治疗前后变化采用配对检验评估；若配对差值近似正态，则使用配对 t 检验，否则使用 Wilcoxon signed-rank 检验。
...
最终监督队列包含 19 例患者。平均年龄为 64.74 岁（SD 6.49），11 例为女性。8 例右侧上肢受累，11 例左侧上肢受累。基线 FMA-UE 为 40.47（SD 23.83），治疗 14 次后 FMA-UE 为 45.47（SD 23.23），观察到的 FMA-UE 改善为 5.00 分（SD 4.07；治疗前后配对 Wilcoxon signed-rank 检验 p < 0.001）。全队列比例恢复残差为 12.87（SD 16.23）。基线 MBI 为 55.53（SD 19.92），治疗后 MBI 为 76.84（SD 21.49；治疗前后配对 t 检验 p < 0.001）。
