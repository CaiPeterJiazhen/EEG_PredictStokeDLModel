...
Author information required before submission: institutional review board name, approval number, consent language, recruitment dates, stroke subtype criteria, time from stroke to EEG, inclusion and exclusion criteria, and whether any standardized conventional rehabilitation was delivered concurrently with tACS.
...
Author information required before submission: repository name and DOI/accession number for shareable code and derived data; whether raw or minimally processed EEG can be shared; data-access contact; data retention policy; consent wording for secondary analysis; and any restrictions imposed by the ethics approval.
...
## Author queries before journal submission

1. Confirm the intervention description: tACS only, conventional rehabilitation, or combined tACS plus rehabilitation.
2. Provide ethics approval institution, approval number, consent wording, and trial or registry information if available.
3. Provide recruitment dates, stroke subtype, lesion-side handling beyond affected-hand alignment, EEG acquisition hardware, electrode montage, preprocessing filters, artifact rejection criteria, and recording duration.
4. Decide which data can be publicly deposited: raw EEG, minimally processed EEG, derived features, subject-level predictions, or tables only.
5. Confirm target journal and reference style so the final reference list can be formatted exactly.
