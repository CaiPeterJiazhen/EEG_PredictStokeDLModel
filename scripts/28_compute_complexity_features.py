...
from eeg_recovery.features.complexity import ComplexityConfig, compute_complexity_for_eeg_record, write_complexity_feature
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute supervised baseline EO/EC raw-EEG Higuchi complexity features.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--config", default=PROJECT_ROOT / "configs" / "paths.example.yaml")
    parser.add_argument("--subject-id", default=None, help="Optional subject filter such as sub01 or sub001.")
    ...
    subject_filter = normalize_subject_id(args.subject_id) if args.subject_id is not None else None
    ...
    selected_records = [
        ...
        and record.stage == BASELINE_STAGE
    ...
    complexity_config = ComplexityConfig(kmax=args.kmax, max_samples=args.max_samples)
    written: list[Path] = []
    for record in selected_records:
        affected_hand = str(labels_by_subject.loc[record.subject_id, "affected_hand"])
        feature = compute_complexity_for_eeg_record(record, affected_hand, config=complexity_config)
        written.append(write_complexity_feature(feature, config.output_root))
